...
(function() {
    ...
    function onFoundAd(streamInfo, textStr, reloadPlayer) {
        ...
        // Below this is the modified code
        // Below this is the modified code
    async function processM3U8(url, textStr, realFetch) {
        ...
        // New Line Added: Prioritizing the highest available stream quality
        ...
        // New addition for better handling of stream quality
        const bestQuality = getBestAvailableQuality(textStr);
        if (bestQuality) {
            textStr = bestQuality;
        }
        return textStr;
    ...
    // New function to prioritize the best stream quality
    function getBestAvailableQuality(textStr) {
        ...
        lines.forEach(line => {
            if (line.startsWith('#EXT-X-STREAM-INF')) {
                const res = parseAttributes(line)['RESOLUTION'];
                if (res && (!bestResolution || parseResolution(res) > parseResolution(bestResolution))) {
                    bestResolution = res;
                    bestQualityUrl = lines[lines.indexOf(line) + 1];
                }
            }
        });
        return bestQualityUrl ? fetch(bestQualityUrl).then(res => res.text()) : null;
    }

    function parseResolution(res) {
        const [width, height] = res.split('x').map(Number);
        return width * height;  // Higher resolution should have a larger value
    }
